...
function docLink(api, search) {
    ...
    return line > 1
        ? `\n\n[View in lua_api.txt](https://github.com/minetest/minetest/blob/5.4.1/doc/lua_api.txt#L${line}-L${
              line + search.split("\n").length - 1
          })`
        : "";
}
...
types.push([
    ...
    (entry, api) => {
        ...
        return {
            ...
            body:
                entry.match(/\[(.+?)`/)[1].replace(/<(.+?)>|(\.\.\.)/g, (arg1, arg2) => `\${${++i}:${arg1 || arg2}}`) +
                "$0",
            desc: entry.match(/^([\S\s]+?)\n*$/)[1] + docLink(api, entry),
            kind: 7,
...
fs.readFile("./lua_api.txt", "utf8", (err, data) => {
    if (!err) {
        fs.writeFile("./smartsnippets.json", JSON.stringify(getSnippets(data)), "utf8", () => {});
    }
});
