...
function makeFiles(files: Array<{ name: string; content: string }>, folders: Array<string>) {
    ...
    let completion = vscode.languages.registerCompletionItemProvider(
        { language: "lua", scheme: "file" },
        {
            provideCompletionItems(document, position) {
                ...
                if (
                    vscode.workspace.getConfiguration("minetest-tools").get("workspaceOnly") &&
                    !(
        ...
        const files = [
            {
                ...
                content: `name = ${
                    name
                        ? name
                        : "".replace(/[_-]/g, " ").replace(/(^| )(\w)/g, function (m) {
